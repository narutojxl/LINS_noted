...
static Eigen::Quaterniond axis2Quat(const Eigen::Vector3d &axis, double theta) {
  ...
  q.w() = cos(theta / 2.0f);
  q.x() = axis(0) * magnitude; //轴角到单位四元数，joan sola 101式
...
static Eigen::Vector3d Quat2axis(const Eigen::Quaterniond &q) {
  ...
  if (axis_magnitude >= 1e-10) {
    ...
    vec = vec * wrap_pi(2.0 * atan2(axis_magnitude, q.w())); //单位四元数到轴角，joan sola 105式
  }
...
static Eigen::Vector3d R2rpy(const Eigen::Matrix3d &R) {
  Eigen::Vector3d rpy;
  rpy(1) = atan2(-R(2, 0), sqrt(R(2, 1) * R(2, 1) + R(2, 2) * R(2, 2)));//jxl: 求的是ry(pitch)
  rpy(0) = atan2(R(2, 1) / cos(rpy(1)), R(2, 2) / cos(rpy(1)));  // roll  rx
  rpy(2) = atan2(R(1, 0) / cos(rpy(1)), R(0, 0) / cos(rpy(1)));  // yaw   rz
  return rpy;
}
